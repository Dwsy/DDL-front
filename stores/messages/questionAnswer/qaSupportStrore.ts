...
import {
    QaSupportType,
    UseAxiosGetQaSupportNotify,
} from '~/composables/Api/messages/qa/support'
...
export const useQaSupportStore = defineStore('QaSupportStore', {
    ...
    actions: {
        async loadQaSupportNotifyList(type: QaSupportType, scroll?: boolean) {
            let {data: response} = await UseAxiosGetQaSupportNotify(
                this.page,
                type
            )
            if (response.code == 0) {
                if (scroll) {
                    this.qaSupportNotifyList = this.qaSupportNotifyList.concat(
