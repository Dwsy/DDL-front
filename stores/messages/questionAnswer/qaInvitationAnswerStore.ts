...
import {UseAxiosGetQaAnswerNotify, useAxiosGetQaInvitationAnswerNotify} from '~/composables/Api/messages/qa/answer'
...
export const useQaInvitationAnswerStore = defineStore('AaInvitationAnswerStore', {
    state: (): State => {
        return {
    ...
    actions: {
        async loadQaInvitationAnswerNotifyList(scroll?: boolean) {
            let {data: response} = await useAxiosGetQaInvitationAnswerNotify(this.page)
            if (response.code == 0) {
                if (scroll) {
                    this.qaInvitationAnswerNotifyList = this.qaInvitationAnswerNotifyList
