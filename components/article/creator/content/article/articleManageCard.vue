<template>
  <div v-for="articleFiled in articleFiledData">
    ...
    <div class="text-medium-emphasis d-article-manage-summary" :title="articleFiled.summary">
      {{ articleFiled.summary }}
    </div>
    <div>
      <v-chip-group>
        <v-chip v-for="tag in articleFiled.articleTags" :key="tag.id"
                size="small">
    ...
    <div class="text-end">
      ...
      <span class="mr-2 text-grey" style="font-size: 16px"
            ...
            v-if="dateFilter(articleFiled.createTime,'YYYY-MM-DD hh:mm')!==dateFilter(articleFiled.lastModifiedTime,'YYYY-MM-DD hh:mm')">
          修改于：{{ timeAgoFilter(articleFiled.lastModifiedTime) }}
        </span>
      <span style="font-size: 16px">创建于：{{ dateFilter(articleFiled.createTime, 'YYYY/MM/DD hh:mm') }}</span>
      <v-btn variant="tonal" class="mx-2" color="#028760" :href="`/article/editor/draft?id=${articleFiled.id}`"
             target="_blank">
        编辑
      </v-btn>
      <v-dialog transition="dialog-bottom-transition">
        <template v-slot:activator="{ props }">
          <v-btn v-bind="props" variant="tonal" color="red">
            删除
          </v-btn>
        </template>
        <template v-slot:default="{ isActive }">
          <v-card max-width="30%" class="mx-auto">
            ...
            <v-card-actions class="justify-end">
              <v-btn
                  variant="tonal" color="#2a6e3f"
                  @click="isActive.value = false"
              >取消
              </v-btn>
              <v-btn
                  variant="tonal" color="#c12c1f"
                  @click="isActive.value = false"
              >删除
              </v-btn>
            </v-card-actions>
