...
onMounted(async () => {
  const {data: newAnswerNumR} = await useGet<ResponseData<number>>('message/notify/count/' + NotifyTypeEn.watch_answer)
  newAnswerCount.value = newAnswerNumR.data
  const {data: acceptedAnswerNumR} = await useGet<ResponseData<number>>('message/notify/count/' + NotifyTypeEn.watch_accepted_question_answer)
