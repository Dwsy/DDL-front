...
import { QaGroup, QuestionData } from '~/types/question'
...
export const useFetchGetNewQuestionPageList = (page: number) => {
  ...
  return http.GET<ResponsePageData<QuestionData>>('question/question/field/list', { page, size:defaultSize })
  // return useGet<ResponseData<any>>('question/question/field/list', {page})
}

export const useFetchGetQuestionField = (questionId: string, getQuestionComment?: boolean) => {
  return http.GET<ResponsePageData<any>>('question/question/field/' + questionId, { getQuestionComment })
}
